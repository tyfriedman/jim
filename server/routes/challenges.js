...
router.get("/", async (req, res, next) => {
  try {
    const result = await withConnection((connection) =>
      connection.execute(
        `SELECT c.challenge_id, c.creator_id, c.exercise_id, e.name AS exercise_name, c.title, c.description,
...
router.post("/", async (req, res, next) => {
  const { exercise_id, title, description, target_value, start_date, end_date, is_public = 1 } = req.body;
  ...
  try {
    ...
    return res.status(201).json({ challenge_id: result.outBinds.challengeId[0] });
  } catch (error) {
    return next(error);
  }
});
